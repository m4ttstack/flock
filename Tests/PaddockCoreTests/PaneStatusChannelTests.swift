...
final class PaneStatusChannelTests: XCTestCase {
    func testBridgeWriteReachesTheAppCallback() throws {
        ...
        // write a `paddock.mouse_capture` line, exactly as `ControlBridge`
    ...
    func testParseMouseCaptureAcceptsBothFlagsAndDefaultsSgr() {
        XCTAssertEqual(
            PaneStatusChannel.parseMouseCapture(Data(#"{"type":"paddock.mouse_capture","enabled":true,"sgr_pixels":true}"#.utf8))?.0, true)
        XCTAssertEqual(
            PaneStatusChannel.parseMouseCapture(Data(#"{"type":"paddock.mouse_capture","enabled":false,"sgr_pixels":true}"#.utf8))?.1, true)
        // sgr_pixels missing defaults to false.
        let parsed = PaneStatusChannel.parseMouseCapture(Data(#"{"type":"paddock.mouse_capture","enabled":true}"#.utf8))
