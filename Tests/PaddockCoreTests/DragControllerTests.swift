...
final class DragControllerTests: XCTestCase {
    ...
    /// The one dwell-only target left: a card's +N tile, which arms like a
    /// thumbnail and fires once its 500ms are up.
    func testDwellingOnAPlusTileFiresItsSpringLoad() async {
        let clock = FakeClock()
        ...
        XCTAssertEqual(springLoad.fired, [.moreTabs(Self.workspaceID)])
    }
