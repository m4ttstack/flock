...
extension XCUIApplication {
    /// Launches Flock against a scratch session. The socket is the point:
    ...
    ///
    /// `FLOCK_HERDR_BIN` and `FLOCK_RESNAPSHOT_SECONDS` are carried across
    ...
    static func flock(socket: String, env: [String: String] = [:]) -> XCUIApplication {
        let app = XCUIApplication()
        app.launchEnvironment["HERDR_SOCKET_PATH"] = socket
        let inherited = ProcessInfo.processInfo.environment
        for key in ["FLOCK_HERDR_BIN", "FLOCK_RESNAPSHOT_SECONDS"] {
    ...
    func flockElement(_ identifier: String) -> XCUIElement {
    ...
    func flockElementCount(identifierPrefix: String) -> Int {
        flockIdentifiers(prefix: identifierPrefix).count
    }
    ...
    func flockIdentifiers(prefix: String) -> [String] {
        flockBoxes(prefix: prefix).keys.sorted()
    }
    ...
    func flockBoxes(prefix: String) -> [String: CGRect] {
    ...
    func flockText(in identifier: String) -> [String] {
        flockText(inAnyOf: [identifier])
    }
    ...
    func flockText(inAnyOf identifiers: [String]) -> [String] {
        guard let tree = try? snapshot() else { return [] }
...
func dragElement(
    ...
    let source = app.flockElement(fromID)
    let target = app.flockElement(toID)
...
func clickElement(
    ...
    let element = app.flockElement(identifier)
