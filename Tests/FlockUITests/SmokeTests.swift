...
final class SmokeTests: XCTestCase {
    ...
    func testAppMirrorsSeedLayout() throws {
        ...
        let p1 = app.flockElement("flock.canvas.pane.\(ids.p1)")
        XCTAssertTrue(p1.waitForExistence(timeout: 60), "the canvas never showed \(ids.p1)")
        let p2 = app.flockElement("flock.canvas.pane.\(ids.p2)")
        XCTAssertTrue(p2.waitForExistence(timeout: 30), "the canvas never showed \(ids.p2)")
        XCTAssertTrue(
            app.flockElement("flock.rail.workspace.\(ids.ws)").waitForExistence(timeout: 30),
            "the rail never showed the seed workspace \(ids.ws)"
        )
        XCTAssertTrue(
            app.flockElement("flock.strip.tab.\(ids.tabA)").waitForExistence(timeout: 30),
            "the strip never showed \(ids.tabA)"
        )
        XCTAssertTrue(app.flockElement("flock.strip.tab.\(ids.tabB)").exists, "the strip never showed \(ids.tabB)")

        // tabB is the seed's unfocused tab, so its pane belongs to no visible
        // cell.
        XCTAssertFalse(
            app.flockElement("flock.canvas.pane.\(ids.p3)").exists,
            "\(ids.p3) is in the tab that is not shown, so the canvas must not hold a cell for it"
        )
        XCTAssertEqual(
            app.flockElementCount(identifierPrefix: "flock.canvas.pane."), truth.paneCount(inTab: ids.tabA),
    ...
    /// runs the whole suite with `FLOCK_RESNAPSHOT_SECONDS` set, so this
    ...
    func testAppConvergesOnAnExternalSplit() throws {
        ...
        XCTAssertTrue(
            app.flockElement("flock.canvas.pane.\(added)").waitForExistence(timeout: 20),
            "the canvas never showed \(added), which herdr added while the app was already running"
        )
        XCTAssertEqual(
            app.flockElementCount(identifierPrefix: "flock.canvas.pane."), 3,
