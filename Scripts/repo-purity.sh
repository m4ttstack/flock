...
#
# flock is headed for a public repo so Sparkle can serve its appcast and
# builds from GitHub Releases. Nothing about any particular employer,
# customer, or internal system belongs in it -- not in code, not in fixtures,
# not in docs, not in design files. This sweeps everything git tracks, so
...
HITS=$(cd "$ROOT" \
  ...
  | grep -vE '^Scripts/repo-purity\.sh:' \
  | grep -vE '^docs/design/src/[^:]*\.html:' || true)
if [ -n "$HITS" ]; then
  echo "FAIL repo-purity:"
  printf '%s\n' "$HITS"
  echo ""
  echo "flock is public. Use neutral placeholders (acme, ACME-1234, gitlab.example.com)."
  exit 1
fi
