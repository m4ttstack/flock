...
`chrome-*.png` and `attention-stack.png` come from the `FlockChromeRender` scheme
rendering the real `MainWindow` offscreen at 900x560 from fixture data, with
`TEST_RUNNER_FLOCK_CHROME_RENDER_DIR` set. `grid-status-dots.png` comes from the
...
| `chrome-tokyo-night.png` | The same window **after**. Every row draws its aggregate status as an 8pt dot: filled yellow / red / teal for working / blocked / done, a hollow green ring for idle. The selected row (`flock`) keeps its own status dot; selection is the row fill and the heavier name. |
...
`.superpowers/sdd/2026-09-10-flock/task-28-renders/` (scratch, untracked).
