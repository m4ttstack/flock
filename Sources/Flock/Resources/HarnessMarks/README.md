...
`Sources/FlockCore/Marks/`, where `VectorMarkPath` decodes it to a `CGPath`
...
- The White variant rather than the Black one: flock's chrome is dark in
...
the harness's name and not flock's branding; it sits on a plain black disc,
which is also what makes the white readable on flock's light themes; and
...
flock is ever distributed beyond personal use, re-read each vendor's current
