...
/// today, `flock.mouse_capture` lines, so the app learns when the pane's
/// program turns mouse reporting on or off. Flock's libghostty never enters
...
public final class PaneStatusChannel {
    ...
    /// Starts reading status lines off the FIFO on `queue`: `flock.
    /// mouse_capture` lines invoke `onCapture(enabled, sgrPixels)`, and the
    /// one-shot `flock.first_frame` line invokes `onFirstFrame`, both in
    ...
    /// `(enabled, sgrPixels)` for a well-formed `flock.mouse_capture` line,
    ...
    public static func parseMouseCapture(_ line: Data) -> (Bool, Bool)? {
        guard
            ...
            object["type"] as? String == "flock.mouse_capture",
    ...
    /// Whether a line is the bridge's one-shot `flock.first_frame` status
    ...
    public static func parseFirstFrame(_ line: Data) -> Bool {
        ...
        return object["type"] as? String == "flock.first_frame"
    }

    /// Whether a line is the bridge's `flock.hold_lost` status line: every
