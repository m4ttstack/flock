...
public enum DividerDragMath {
    ...
    /// itself never enforced one; only flock's own clamp does, and only on
    ...
    /// desync `SurfaceGrid` warns about. Flock clamps in cells too, to
...
/// (FlockCore) for the arm/release latch rather than reimplementing it: a
