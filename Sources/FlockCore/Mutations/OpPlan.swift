...
public enum DropTarget: Equatable, Sendable {
    ...
    /// A grid card's tile: "+N" at rest, "fewer" once expanded. Nothing lands
    /// on either; a dwell on the "+N" one expands the card so its hidden tabs
    /// can take the drop instead.
...
private enum PlanPlaceholder {
    static let createdTabPrefix = "\u{0}flock.plan.createdTab."
    static let movedPanePrefix = "\u{0}flock.plan.movedPane."
