...
/// Maps paddock's own `Theme` (17 herdr palettes, `Sources/Paddock/Theme/Theme.swift`)
/// into the color slots `GhosttyThemeConfig` needs. Lives here, not in
/// `PaddockCore`, because it needs `NSColor` to read a `SwiftUI.Color`'s bytes
/// back out; `GhosttyThemeConfig`'s own text generation is pure and pinned by
/// `GhosttyThemeConfigTests` in `PaddockCoreTests` against hand-computed
...
extension Theme {
    ...
    /// (Black -> Surface1), the palette family paddock's default theme most
