...
/// `updateNSView`, same as `TerminalRepresentable`). `surface` is the
/// type-erased `GhosttyPaneSurface` `SessionViewModel` hands back from
/// `beginOrUpdateGhosttyAttach` -- downcast here, in the app layer, to reach
/// the real `GhosttySession` PaddockCore is never allowed to see.
