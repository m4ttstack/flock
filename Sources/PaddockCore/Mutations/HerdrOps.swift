...
/// direction form (paddock always names both panes).
...
/// (the alternative `pane_id` form is unused: paddock always knows the tab).
...
/// present only when `changed` is false; paddock's explicit-pair calls can
...
/// Shared for every verb whose response carries nothing paddock needs
...
/// still fails loudly, without paddock caring about the payload shape.
